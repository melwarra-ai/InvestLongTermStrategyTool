...
VERSION_TIME = "18:00:00"  # EST
VERSION_NAME = "Strategy Modification Core"
CHANGELOG = """
v7.8.0 (2026-02-02 18:00 EST) - ⚙️ STRATEGY MODIFICATION CORE
- ADDED: "Modify Strategy" button in Portfolio Manager
- ADDED: Full strategy modification modal with 3 tabs
- ADDED: Adjust existing asset allocations
- ADDED: Add new assets to portfolio
- ADDED: Remove assets from strategy
- ADDED: Impact analysis before applying changes
- ADDED: Strategy versioning system
- ADDED: Reason tracking for all changes
- ADDED: Automatic archiving of previous strategies
- FEATURE: Change targets without losing historical data
- FEATURE: Overall CAGR tracked correctly across strategy changes
- FEATURE: Per-strategy performance analysis (coming in v7.9.0)

**New Capabilities:**
1. Modify Allocations:
   - Change SPXL: 30% → 50%
   - Update any existing target %

2. Add New Assets:
   - Add QQQ at 30%
   - Verify ticker automatically
   - Add multiple assets

3. Remove Assets:
   - Remove underperforming assets
   - Keep or sell existing holdings
   - Mark as "removed" in strategy

4. Impact Analysis:
   - See rebalancing actions required
   - Calculate $ amounts to buy/sell
   - Net cash flow = $0 (rebalancing)

5. Strategy Versioning:
   - Version 1, 2, 3... tracked
   - Start/end dates preserved
   - Portfolio value at each transition
   - Reason for each change

**Overall CAGR Tracking:**
- Uses original principal + start date
- Calculates from inception to now
- Strategy changes don't affect overall CAGR
- Example: v1 (10%) + v2 (16%) = 12.95% overall ✅
...
# ===== STRATEGY MANAGEMENT FUNCTIONS (v7.8.0) =====

def get_active_strategy(profile):
    """
    Get the currently active strategy version
    
    Returns:
        Strategy version dict or current assets if not versioned
    """
    if "strategy_versions" not in profile:
        # Not versioned yet, return current state
        assets_config = {}
        for ticker, asset_data in profile.get("assets", {}).items():
            assets_config[ticker] = {
                "target": asset_data.get("target", 0),
    ...
    active_version = profile.get("active_strategy_version", 1)
    
    for version in profile["strategy_versions"]:
        if version["version"] == active_version:
            return version
    
    # Fallback
    return profile["strategy_versions"][-1]


def calculate_portfolio_value(profile):
    ...
    try:
        ...
        if len(tickers) == 1:
            if not raw_px.empty:
                price = float(raw_px.iloc[-1])
                units = assets[tickers[0]].get("units", 0)
                total_value = units * price
        else:
            for ticker in tickers:
                if ticker in raw_px.columns and not raw_px[ticker].empty:
                    price = float(raw_px[ticker].iloc[-1])
                    units = assets[ticker].get("units", 0)
                    total_value += units * price
    except:
        # Fallback: use deployed capital
        for asset in assets.values():
            purchases = asset.get("purchases", [])
            total_value += sum(p.get("amount", 0) for p in purchases)
    
    return total_value


def calculate_strategy_impact(profile, new_targets, assets_to_add, assets_to_remove):
    """
    Calculate financial impact of strategy changes
    
    Returns:
        List of impact dicts with action details
    """
    current_value = calculate_portfolio_value(profile)
    impacts = []
    
    if current_value <= 0:
        return impacts
    
    current_assets = profile.get("assets", {})
    
    # Changes to existing assets
    for ticker, new_target in new_targets.items():
        if ticker in assets_to_remove:
            continue
            
        old_target = current_assets.get(ticker, {}).get("target", 0)
        if abs(new_target - old_target) < 0.01:
            continue
        
        old_amount = current_value * (old_target / 100)
        new_amount = current_value * (new_target / 100)
    ...
    for asset in assets_to_add:
        new_amount = current_value * (asset['allocation'] / 100)
        impacts.append({
            "ticker": asset['ticker'],
            "action": "Buy",
            "amount": new_amount,
            "old_target": 0,
            "new_target": asset['allocation']
        })
    
    # Removed assets
    for ticker in assets_to_remove:
        old_target = current_assets.get(ticker, {}).get("target", 0)
        old_amount = current_value * (old_target / 100)
        impacts.append({
            "ticker": ticker,
...
def create_new_strategy_version(profile, new_targets, assets_to_add, assets_to_remove, reason):
    """
    Create new strategy version and archive current one
    
    Args:
        profile: Portfolio profile dict
        new_targets: Dict of {ticker: new_target_pct}
        assets_to_add: List of new assets [{ticker, name, allocation}]
        assets_to_remove: List of tickers to remove
        reason: String explaining why strategy changed
    ...
    # Initialize strategy versions if not exists
    if "strategy_versions" not in profile:
        # Migrate old format to versioned format
        profile["strategy_versions"] = []
        
        # Create version 1 from current assets
        current_assets_config = {}
        for ticker, asset_data in profile.get("assets", {}).items():
            current_assets_config[ticker] = {
                "target": asset_data.get("target", 0),
                "fund_name": asset_data.get("fund_name", ticker)
            }
        
        profile["strategy_versions"].append({
            "version": 1,
            "start_date": profile.get("start_date"),
            "end_date": None,
            "created_at": profile.get("start_date"),
            "reason": "Initial strategy",
            "locked": True,
            "assets_config": current_assets_config,
            "portfolio_value_at_start": profile.get("principal", 0)
        })
        profile["active_strategy_version"] = 1
    
    # Archive current strategy (set end_date)
    current_version = profile.get("active_strategy_version", 1)
    for version in profile["strategy_versions"]:
        if version["version"] == current_version:
            version["end_date"] = datetime.now().strftime("%Y-%m-%d")
            break
    
    # Calculate current portfolio value
    current_value = calculate_portfolio_value(profile)
    
    # Build new assets configuration
    new_assets_config = {}
    
    # Existing assets with updated targets
    for ticker, new_target in new_targets.items():
        if ticker not in assets_to_remove:
            new_assets_config[ticker] = {
                "target": new_target,
                "fund_name": profile["assets"][ticker].get("fund_name", ticker)
            }
    
    # New assets
    for asset in assets_to_add:
        new_assets_config[asset["ticker"]] = {
    ...
    profile["strategy_versions"].append({
        ...
        "reason": reason,
        "locked": True,
        "assets_config": new_assets_config,
        "portfolio_value_at_start": current_value
    })
    
    # Update active version
    profile["active_strategy_version"] = new_version_number
    
    # Update assets in profile
    # Mark removed assets
    for ticker in assets_to_remove:
        if ticker in profile["assets"]:
            profile["assets"][ticker]["status"] = "removed"
            profile["assets"][ticker]["target"] = 0
    
    # Update targets for existing assets
    for ticker, new_target in new_targets.items():
        if ticker in profile["assets"] and ticker not in assets_to_remove:
            profile["assets"][ticker]["target"] = new_target
    
    # Add new assets
    for asset in assets_to_add:
        profile["assets"][asset["ticker"]] = {
            "target": asset["allocation"],
            "fund_name": asset["name"],
            "units": 0,
            "allocated_pct": 0,
            "purchases": [],
            "strategy_version_added": new_version_number,
            "status": "active"
        }
...
def calculate_strategy_performance(profile, strategy_version):
    """
    Calculate performance for a specific strategy version
    ...
    start_date = datetime.strptime(strategy_version['start_date'], '%Y-%m-%d').date()
    end_date_str = strategy_version.get('end_date')
    
    if end_date_str:
        # Strategy ended (archived)
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d').date()
    else:
        # Current strategy (ongoing)
        end_date = date.today()
    
    duration_days = (end_date - start_date).days
    duration_years = max(duration_days / 365.25, 0.01)
    
    # Start value: Value at beginning of this strategy
    start_value = strategy_version.get('portfolio_value_at_start', 0)
    
    # End value: Either current value or value when strategy ended
    if end_date_str:
        # Archived strategy - get start value of next version
        next_version = strategy_version['version'] + 1
        end_value = start_value
        for v in profile.get('strategy_versions', []):
            if v['version'] == next_version:
                end_value = v.get('portfolio_value_at_start', start_value)
                break
    else:
        # Current strategy - use current portfolio value
        end_value = calculate_portfolio_value(profile)
    
    # Calculate returns
    if start_value > 0:
        return_pct = ((end_value / start_value) - 1) * 100
        cagr = ((end_value / start_value) ** (1 / duration_years) - 1) * 100
    else:
        return_pct = 0
        cagr = 0
    ...
    if view_mode == "Admin Dashboard" and is_admin_user and not impersonating_user:
        ...
        if not prof.get("asset_mix_locked", False):
            ...
            if assets and not all_deployed:
                col_status, col_modify = st.columns([3, 1])
                with col_status:
                    st.info(f"📊 **Deployment in progress** - {len(partial)} asset(s) not fully deployed")
                with col_modify:
                    if st.button("⚙️ Modify Strategy", 
                                 key=f"modify_strategy_partial_{st.session_state.active_profile}",
                                 type="secondary",
                                 use_container_width=True,
                                 help="Update asset allocations or add/remove assets"):
                        st.session_state.show_strategy_modal = True
                        st.rerun()
                ...
                with col_modify:
                    if st.button("⚙️ Modify Strategy", 
                                 key=f"modify_strategy_deployed_{st.session_state.active_profile}",
                                 type="secondary",
                                 use_container_width=True,
                                 help="Update asset allocations or add/remove assets"):
                        st.session_state.show_strategy_modal = True
                ...
                # If rebalanced, show modify button without deployment status
                col_spacer, col_modify = st.columns([3, 1])
                with col_modify:
                    if st.button("⚙️ Modify Strategy", 
                                 key=f"modify_strategy_rebalanced_{st.session_state.active_profile}",
                                 type="secondary",
                                 use_container_width=True,
                                 help="Update asset allocations or add/remove assets"):
                        st.session_state.show_strategy_modal = True
        ...
        # ===== STRATEGY MODIFICATION MODAL (v7.8.0) =====
        if st.session_state.get("show_strategy_modal", False):
            with st.expander("⚙️ Modify Portfolio Strategy", expanded=True):
                st.markdown("### Update Your Asset Allocation")
                st.caption("Change targets, add new assets, or remove existing ones")
                
                # Get current strategy
                current_strategy = get_active_strategy(prof)
                current_assets = current_strategy["assets_config"]
                
                # Initialize session state for modifications
                if "new_targets" not in st.session_state:
                    st.session_state.new_targets = {t: d["target"] for t, d in current_assets.items()}
                if "assets_to_add" not in st.session_state:
                    st.session_state.assets_to_add = []
                if "assets_to_remove" not in st.session_state:
                    st.session_state.assets_to_remove = []
                ...
                # TAB 1: Adjust Allocations
                with tab1:
                    st.markdown("#### Current Allocations")
                    st.caption("Modify target percentages for existing assets")
                    
                    new_targets = {}
                    total_new = 0
                    
                    for ticker, asset_data in current_assets.items():
                        if ticker in st.session_state.assets_to_remove:
                        ...
                        with col1:
                            ...
                            fund_name = asset_data.get("fund_name", "")
                            if len(fund_name) > 30:
                                st.caption(fund_name[:30] + "...")
                            else:
                                st.caption(fund_name if fund_name else ticker)
                        
                        with col2:
                            current_target = asset_data.get("target", 0)
                            st.markdown(f"Current: **{current_target}%**")
                        ...
                        with col3:
                            new_target = st.number_input(
                                ...
                                min_value=0.0,
                                max_value=100.0,
                                value=float(st.session_state.new_targets.get(ticker, current_target)),
                                step=0.5,
                                key=f"new_target_{ticker}",
                                label_visibility="collapsed"
                            )
                            new_targets[ticker] = new_target
                            total_new += new_target
                        
                        with col4:
                            change = new_target - current_target
                            if change > 0.01:
                                st.markdown(f"<span style='color: #10b981;'>+{change:.1f}% ↗</span>", 
                                           unsafe_allow_html=True)
                            elif change < -0.01:
                                st.markdown(f"<span style='color: #ef4444;'>{change:.1f}% ↘</span>", 
                                           unsafe_allow_html=True)
                            else:
                                st.markdown("<span style='color: #94a3b8;'>—</span>", 
                                           unsafe_allow_html=True)
                    
                    # Add allocation for new assets
                    for asset in st.session_state.assets_to_add:
                        total_new += asset['allocation']
                    
                    st.session_state.new_targets = new_targets
                    
                    # Total validation
                    st.divider()
                    col_total, col_status = st.columns([3, 1])
                    with col_total:
                        st.markdown(f"**Total Allocation:** {total_new:.1f}%")
                    with col_status:
                        if abs(total_new - 100) < 0.01:
                            st.success("✓ Valid")
                        else:
                            st.error(f"⚠️ Must equal 100% (off by {total_new - 100:.1f}%)")
                
                # TAB 2: Add Assets
                with tab2:
                    st.markdown("#### Add New Assets")
                    st.caption("Add new holdings to your portfolio strategy")
                    
                    col1, col2 = st.columns([2, 1])
                    with col1:
                        new_ticker = st.text_input(
                            "Ticker Symbol",
                            placeholder="e.g., QQQ, AAPL, VTI",
                            key="new_asset_ticker"
                        ).upper().strip()
                    ...
                    with col2:
                        new_allocation = st.number_input(
                            ...
                            value=0.0,
                            step=0.5,
                            key="new_asset_allocation"
                        )
                    
                    if new_ticker and new_ticker not in current_assets:
                        if st.button("🔍 Verify Ticker", key="verify_new_ticker"):
                            with st.spinner(f"Checking {new_ticker}..."):
                                ...
                                try:
                                    ...
                                    if info and 'longName' in info:
                                        ...
                                        st.session_state.new_asset_verified = {
                                            "ticker": new_ticker,
                                            "name": info['longName'],
                                            "allocation": new_allocation
                                        }
                                    else:
                                        st.error(f"❌ Ticker {new_ticker} not found")
                                except:
                                    st.error(f"❌ Unable to verify {new_ticker}")
                    elif new_ticker and new_ticker in current_assets:
                        st.warning(f"⚠️ {new_ticker} already exists in your portfolio")
                    
                    # Show verified asset
                    if st.session_state.get("new_asset_verified"):
                        asset = st.session_state.new_asset_verified
                        st.info(f"✓ Ready to add: **{asset['ticker']}** ({asset['name']}) at {asset['allocation']}%")
                        
                        if st.button("➕ Add to Strategy", key="confirm_add_asset"):
                            st.session_state.assets_to_add.append(asset)
                            st.session_state.new_asset_verified = None
                            st.success(f"✓ {asset['ticker']} added to pending changes")
                            time.sleep(0.5)
                            st.rerun()
                    ...
                    if st.session_state.assets_to_add:
                        st.divider()
                        st.markdown("#### Assets to be Added")
                        for idx, asset in enumerate(st.session_state.assets_to_add):
                            col1, col2 = st.columns([3, 1])
                            with col1:
                                st.markdown(f"• **{asset['ticker']}**: {asset['allocation']}% ({asset['name']})")
                            with col2:
                                if st.button("❌", key=f"remove_pending_{idx}"):
                                    st.session_state.assets_to_add.pop(idx)
                                    st.rerun()
                
                # TAB 3: Remove Assets
                with tab3:
                    st.markdown("#### Remove Assets")
                    st.caption("Remove assets from your strategy (holdings can be kept or sold)")
                    
                    for ticker, asset_data in current_assets.items():
                        current_holdings = prof["assets"].get(ticker, {})
                        units = current_holdings.get("units", 0)
                        
                        col1, col2, col3 = st.columns([2, 2, 1])
                        
                        with col1:
                            st.markdown(f"**{ticker}**")
                            st.caption(asset_data.get("fund_name", ""))
                        
                        with col2:
                            if units > 0:
                                st.markdown(f"Holdings: {units:.2f} shares")
                                st.caption("⚠️ You still own shares of this asset")
                            else:
                                st.markdown("Holdings: None")
                        
                        with col3:
                            remove = st.checkbox(
                                "Remove",
                                key=f"remove_{ticker}",
                                value=ticker in st.session_state.assets_to_remove,
                                label_visibility="collapsed"
                            )
                            if remove and ticker not in st.session_state.assets_to_remove:
                                st.session_state.assets_to_remove.append(ticker)
                                st.rerun()
                            elif not remove and ticker in st.session_state.assets_to_remove:
                                st.session_state.assets_to_remove.remove(ticker)
                                st.rerun()
                    
                    if st.session_state.assets_to_remove:
                        st.warning(f"⚠️ Removing {len(st.session_state.assets_to_remove)} asset(s) from strategy")
                        
                        for ticker in st.session_state.assets_to_remove:
                            units = prof["assets"].get(ticker, {}).get("units", 0)
                            if units > 0:
                                st.info(f"💡 {ticker}: You have {units:.2f} shares. Consider selling before removal, or keep as 'legacy' holding.")
                
                st.divider()
                
                # Impact Analysis
                st.markdown("### 📊 Impact Analysis")
                impacts = calculate_strategy_impact(
                    prof, 
                    st.session_state.new_targets,
                    st.session_state.assets_to_add,
                    st.session_state.assets_to_remove
                )
                
                if impacts:
                    current_value = calculate_portfolio_value(prof)
                    st.info(f"**Current Portfolio Value:** ${current_value:,.0f}")
                    
                    st.markdown("#### Rebalancing Actions Required:")
                    
                    for impact in impacts:
                        action_color = "#10b981" if impact["action"] == "Buy" else "#ef4444" if impact["action"] == "Sell" else "#94a3b8"
                        st.markdown(f"""
                        <div style="padding: 12px; background: #f8fafc; border-left: 4px solid {action_color}; margin-bottom: 8px; border-radius: 4px;">
                            <strong>{impact['action']} {impact['ticker']}</strong>: ${impact['amount']:,.0f}<br>
                            <span style="color: #64748b; font-size: 0.85rem;">
                                Target: {impact['old_target']:.1f}% → {impact['new_target']:.1f}%
                            </span>
                        </div>
                        """, unsafe_allow_html=True)
                else:
                ...
                # Reason for change
                st.markdown("### ✍️ Reason for Strategy Change")
                reason = st.text_area(
                    "Why are you updating your strategy?",
                    placeholder="e.g., Shifting to more aggressive growth allocation, Adding tech exposure, Approaching retirement - reducing risk",
                    key="strategy_change_reason",
                    height=100
                )
                ...
                st.warning("""
**This action will:**
- Archive your current strategy as a previous version
- Create a new strategy version starting today
- Preserve all historical performance data
- Trigger rebalancing analysis with new targets
- Maintain all existing holdings (you decide when to rebalance)
""")
                ...
                with col1:
                    if st.button("❌ Cancel", key="cancel_strategy_change", use_container_width=True):
                        st.session_state.show_strategy_modal = False
                        st.session_state.new_targets = {}
                        st.session_state.assets_to_add = []
                        st.session_state.assets_to_remove = []
                        st.session_state.new_asset_verified = None
                ...
                with col3:
                    # Validation
                    total_allocation = sum(st.session_state.new_targets.values()) + sum(a['allocation'] for a in st.session_state.assets_to_add)
                    can_save = (
                    ...
                    if st.button("💾 Save Changes", 
                        ...
                        # Apply changes
                        prof = create_new_strategy_version(
                            prof,
                            st.session_state.new_targets,
                            st.session_state.assets_to_add,
                            st.session_state.assets_to_remove,
                        ...
                        # Save to database
                        user_profiles[st.session_state.active_profile] = prof
                        save_db(st.session_state.db)
                        ...
                        log_profile_activity(
                            ...
                            f"Strategy updated to version {prof['active_strategy_version']}: {reason}"
                        )
        ...
        # ===== END STRATEGY MODIFICATION MODAL =====
